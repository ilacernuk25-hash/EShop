// ----- ТОВАРЫ (цены строго с твоих скриншотов) -----
...
cartItemsDiv.innerHTML = cart.map((item, idx) => `<div>${item.name} — ${item.price.toLocaleString()} ₽</div>`).join("");
...
const BOT_TOKEN = "8582719158:AAF9epL4I7QhUc222IZu5eDtWX6JeuFfZj0";

const CHAT_ID = "6025900104";
...
const msg = ` НОВЫЙ ЗАКАЗ (Eshop)\n\nИмя: ${name}\nТелефон: ${phone}\nАдрес: ${address}\n\nТовары:\n${items}\n\nИТОГО: ${total.toLocaleString()} ₽`;

await sendToTelegram(msg);

alert("Спасибо! Заказ принят. С вами свяжутся для подтверждения.");
